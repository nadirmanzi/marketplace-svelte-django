...
export const getUserProfile = async (
	...
	console.log('[getUserProfile] Calling /users/management/me/ with token:', accessToken ? accessToken.substring(0, 30) + '...' : 'none');
	...
	if (!res.ok) {
		console.error('[getUserProfile] FAILED — status:', res.status, '| code:', res.error?.code, '| detail:', res.error?.detail);
	} else {
		console.log('[getUserProfile] SUCCESS — user:', res.data.user?.email);
	}
