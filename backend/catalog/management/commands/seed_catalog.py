...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not admin:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Created default admin (admin@example.com) to own products."
                )
            )
        else:
        ...
        self.stdout.write(
            self.style.SUCCESS("Mock data generation successfully completed!")
        )

    def _generate_data(self, admin):
        # 1. Create Core Categories
        self.stdout.write("Creating categories...")
        tech = self.create_category(
            "Technology (Mock)", "electronics, computers, and tech gadgets."
        )
        apparel = self.create_category(
            "Apparel (Mock)", "clothing, shoes, and lifestyle apparel."
        )
        home = self.create_category(
            "Home & Kitchen (Mock)", "furniture, tools, and kitchen appliances."
        )

        # Subcategories
        laptops = self.create_category(
            "Laptops (Mock)", "High performance computers", tech
        )
        wearables = self.create_category(
            "Wearables (Mock)", "Watches and fitness trackers", tech
        )
        ...
        for color in colors:
            for size in sizes:
                ...
                base_price = (
                    Decimal("140.00") if size not in ["11"] else Decimal("145.00")
                )
                VariantService.create_variant(
                    performed_by=admin,
                    product=shoe,
        ...
        self.stdout.write(
            f"Generated {Category.objects.count()} categories and successfully seeded all products/variants."
        )

    def create_category(self, name, description, parent=None):
        cat, created = Category.objects.get_or_create(
            name=name, defaults={"description": description, "parent": parent}
        )
